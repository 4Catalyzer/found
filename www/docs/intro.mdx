...
Found enables "Render-as-You-Fetch" patterns for efficient code splitting and data fetching in your React applications with or without Suspense.
This gives you matched control over those loading states, even for routes with code bundles that have not yet been downloaded.

Found is designed to be extremely customizable.
Most pieces of Found such as the path matching algorithm and the route element resolution can be fully replaced.
This allows [extensions](/advanced/further-reading) such as [Found Relay](https://github.com/4Catalyzer/found-relay) to
provide first-class support for different use cases.

Found has first class integration with [Redux](https://redux.js.org/) for state management and [Farce](https://github.com/4Catalyzer/farce) for
controlling browser navigation. It can integrate with your existing store and connected components.
    ...
    <Route path="/" Component={AppPage}>
      ...
      <Route path="widgets">
        ...
        <Route
          path=":widgetId"
          getComponent={() =>
            import('./WidgetPage').then((module) => module.default)
          }
